...
class AncestresPdf:
    def __init__(self, nomFichierSortie, max_h, max_v, pos_2, filigrane, titre, image, statistiques):
        # max_h = max horizontal, max_v = max vertical, pos_2 = position verticale du pere (pour echapper le cartouche)
        page_x = M_G + (REC_X + ESP_X) * max_h + M_D
        self.page_y = M_H + (REC_Y + ESP_Y) * max_v + REC_Y + M_B
        ...
        y_2 = self.page_y - M_H - (REC_Y + ESP_Y) * pos_2
    ...
    def dessineAncestre(self, rang_h, rang_v, nom, prenom, dateN, lieuN, dateD, lieuD):
        ...
        x = M_G + (REC_X + ESP_X) * rang_h
        y = self.page_y - self.offset_y - M_H - REC_Y - (REC_Y + ESP_Y) * rang_v
        #print (prenom, x, y)
        ...
        self.ejcritLigneLimitee(texte, u'%s %s'%(nom, prenom), 'Helvetica-Bold', P_NOM)
        if dateN != '' or lieuN != '': self.ejcritLigneLimitee(texte, u'n %s %s'%(dateN, lieuN), 'Helvetica', P_DATE)
        else: texte.textLine(u'')
        if dateD != '' or lieuD != '': self.ejcritLigneLimitee(texte, u'\u2020 %s %s'%(dateD, lieuD), 'Helvetica', P_DATE)
        #texte.setFont("Helvetica-Bold", P_NOM)
        #texte.textLine(u'%s %s'%(nom, prenom))
        #texte.setFont("Helvetica", P_DATE)
        #texte.textLine(u'n %s %s'%(dateN, lieuN))
        #texte.textLine(u'\u2020 %s %s'%(dateD, lieuD))
    ...
    #dessine le lien entre un ancêtre et son père et sa mère, s'ils existent
    def dessineLien(self, rang_h, rang_v, rang_v_p, rang_v_m):
        #rien à faire si ni père ni mère
        if rang_v_p == -1 and rang_v_m == -1: return
        #dessine le segment en partance vers les parents
        x1 = M_G + (REC_X + ESP_X) * rang_h + REC_X
        y1 = self.page_y - self.offset_y - M_H - REC_Y - (REC_Y + ESP_Y) * rang_v + REC_Y / 2
        x2 = x1 + ESP_X /2
        #y2 = y1
        ...
        if rang_v_p != -1:
            #x3 = x2
            y3 = self.page_y - self.offset_y - M_H - REC_Y - (REC_Y + ESP_Y) * rang_v_p + REC_Y / 2
            self.can.line(x2, y1, x2, y3)
            self.can.line(x2, y3, x4, y3)
        if rang_v_m != -1:
            #x5 = x2
            #x6 = x4
            y5 = self.page_y - self.offset_y - M_H - REC_Y - (REC_Y + ESP_Y) * rang_v_m + REC_Y / 2
            self.can.line(x2, y1, x2, y5)
            self.can.line(x2, y5, x4, y5)
            
    #écrit la date et le lieu du mariage devant le rectangle du descendant
    def ejcritMariage(self, rang_h, rang_v_e, dateMariage, lieuMariage):
        #calcule le point d'écriture
        x = M_G + (REC_X + ESP_X) * rang_h + MG_REC
        y = self.page_y - self.offset_y - M_H - REC_Y - (REC_Y + ESP_Y) * rang_v_e + REC_Y / 2 - 4
        texte = self.can.beginText()
